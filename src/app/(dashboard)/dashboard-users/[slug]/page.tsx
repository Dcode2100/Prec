...
import { columnsForDashboardEvents } from '../components/columns'
import FilterDrawer from '../components/filterDrawerForEvents'
...
import { getEventByUserId } from '@/lib/api/dashboardUsersApi'
...
const DashboardEventsPage = () => {
  ...
  const fetchOrder = async () => {
    ...
    const data = await getEventByUserId(slug as string, params)
    return data
  }
  ...
  const renderUserName = () => {
    ...
    if (!dashboardEvents?.events.length) {
      return (
    ...
    const { first_name, last_name } = dashboardEvents.events[0]
    return (
      <span className="text-highlight-blue text-3xl font-semibold">
        {first_name} {last_name}&apos;s events
      </span>
    )
  }
