              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-lg font-semibold">
                    {detail.split_from} : {detail.split_to}
                  </span>
                </div>
                <div className="flex items-center space-x-2">
                  <span className="text-sm font-medium text-muted-foreground">
                  ...
                  <span className="text-base">
                    {format(new Date(detail.record_date), 'MMMM d, yyyy')}
                  </span>
                </div>
  ...
  return (
    <Breadcrumb className="mb-4">
      <BreadcrumbList>
        ...
        <BreadcrumbSeparator />
        <BreadcrumbItem>
          <BreadcrumbLink>Update Asset</BreadcrumbLink>
        </BreadcrumbItem>
      </BreadcrumbList>
    </Breadcrumb>
  )
...
const EquityDetailsForm = () => {
  ...
  const queryKey = useMemo(() => ['tokensData ' + slug, slug], [slug])
  const { data: selectedAsset } = useQuery({
    queryKey,
    queryFn: () => fetchAccounts(slug as string),
  })
  ...
  useEffect(() => {
    if (selectedAsset) {
      ...
      setLogoUrl(selectedAsset.logo)
      setLogoMarkUrl(selectedAsset.logoMark)
      setMobileLogoUrl(selectedAsset.mobileLogo)
      ...
      const convertLogos = () => {
        ...
        if (selectedAsset.logo) {
          handleLogo(selectedAsset.logo, setLogoBlobImage)
        }
        if (selectedAsset.logoMark) {
          handleLogo(selectedAsset.logoMark, setLogoMarkBlobImage)
        }
        if (selectedAsset.mobileLogo) {
          handleLogo(selectedAsset.mobileLogo, setMobileLogoBlobImage)
        }
      }
  ...
  return (
    <div className="w-full px-10  overflow-auto ">
      <NewAssetBreadcrumb />
      <div className="h-full space-y-2 mb-[50px]">
        <Formik
