...
import { WishlistResponse, WishlistsParams } from '@/lib/types/types'
...
import { UploadWishlistData } from '@/lib/api/wishlistApi'

import { useToast } from '@/hooks/use-toast'
import { getWishlistData, updateWishlist } from '@/lib/api/wishlistApi'
import { columnsForWishlist } from './components/table/columns'
import { CSVLink } from 'react-csv'
import { wishlistHeaders } from '@/constants/headers'
...
const ActionButton = ({ row }: { row: WishlistResponse }) => {
  ...
  const updateNotifiedStatus = async (value: string) => {
    setIsUpdating(true)
    try {
      const response = await updateWishlist(row.wishlist_id, value === 'yes')
      if (response.data) {
        toast({
...
const Wishlist = () => {
  ...
  const fetchOrder = async () => {
    ...
    const data = await getWishlistData(params)
    return data
  }
  ...
  const {
    ...
    queryKey: ['all-wishlists', dateFilter, selectedToken, search],
    queryFn: () => fetchOrder(),
  })
  ...
  const handleBulkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      if (e.target.files[0]?.type === 'text/csv') {
        ...
        try {
          const upload = await UploadWishlistData(formData)
  ...
  return (
    <div className="w-full h-full flex overflow-auto flex-col items-center justify-start">
      ...
      <div className="w-full px-4">
        <DataTable
          columns={getColumns() as ColumnDef<WishlistResponse>[]}
          data={getFilteredData()}
          enableSearch={true}
